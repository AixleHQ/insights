...
help:
	...
	@echo "  make test-tools     - Run @aixle/insights tests (Vitest)"
	@echo "  make build-tools    - Build @aixle/insights (TypeScript)"
	@echo ""
	@echo "Code Quality:"
...
# test-tools is intentionally excluded here — it runs on the host, not in a
# container (see its own comment below); run `make test-tools` separately.
...
# Runs on the host, not in a container: the tools workspace is not mounted into
# any compose service, and better-sqlite3 needs a native binding built for the
# local Node.
test-tools:
	cd packages/tools && npm test --workspace=@aixle/insights

build-tools:
	cd packages/tools && npm run build --workspace=@aixle/insights
