...
- The "last-seen model wins" semantic at `199-200` is **load-bearing for cost accuracy**. In an advisor session (Sonnet executor + Opus advisor turns, see §2.9 of `DATA-CLAUDE.md`), the `metadata.model` will be whichever model was last in the JSONL when sync ran. `pricing.ts:33-111` then bills *all* tokens at that model's rate, even though the actual mix was split. `pricing.ts` header comment acknowledges this as a known approximation.
...
| `advisor_call` | A turn where `entry.message.model` is the configured Opus model id and the prior assistant turn used the Sonnet executor (per DATA-CLAUDE.md §2.9 model-flip detection) | JSONL assistant turn boundary | Captures the Sonnet→Opus escalation. Pairs with `metadata.advisor_model` (see §10c). |
...
These are all derivable from `DATA-CLAUDE.md` §2.1, §2.4, §2.6, §2.8, §2.9, §2.12, but none are persisted today.
...
| `advisor_model` | string (`claude-opus-4-7`, etc.) | `entry.message.model` on advisor turns (model-flip per DATA-CLAUDE.md §2.9) | Not extracted | Detect model-flips during aggregation; record the advisor model id separately from the executor's `model` |
...
- `packages/web/src/lib/types.ts:214-222` — extend the `EventType` union with `tool_use`, `subagent_run`, `skill_run`, `hook_event`, `advisor_call`, `mcp_tool_call`, `extended_thinking` (in addition to the cursor-side values listed in §4). After both passes, the union becomes the canonical client-side enum and `EventFilters.tsx` should import from it.
- `packages/web/src/lib/types.ts` — extend the `ToolEventMetadata` interface (or introduce one if it doesn't yet exist) with the typed sub-shape for claude metadata keys: `entrypoint?: "cli" | "sdk" | "ide" | "claude_p"`, `advisor_model?: string`, `git_branch?: string`, `iterations?: number`, `stop_reason_summary?: Record<StopReason, number>`, `service_tier?: "standard" | "priority" | "flex"`, `claude_code_version?: string`, `subagent_type?: string`, `skill_name?: string`, `tool_name_inner?: string`, `cache_ttl_split?: { ephemeral_5m: number; ephemeral_1h: number }`, `request_id?: string`, `cost_source?: "client" | "server_estimated"`. This unblocks typed access in dashboard tiles instead of `unknown` casts.
...
- Dashboard surfaces — once `subagent_run` / `skill_run` / per-tool granularity lands, the org dashboard (`packages/web/src/pages/OrgDashboard.tsx`) and the events page deserve dedicated tiles: an "Advisor cost" KPI (Sonnet+Opus cost decomposition), a "Tool-use breakdown" pie (`tool_name_inner` distribution), and a "Refusal rate" alert. None of these require schema additions beyond §10c.
...
| `metadata.advisor_model` | No (model id) | No | `claude-opus-4-7` / `claude-sonnet-4-6` — bounded model ids. |
...
| `claude-3` | Extractor | Surface per-session metadata: `entrypoint`, `advisor_model`, `git_branch`, `cwd`, `iterations`, `stop_reason_summary`, `service_tier`, `claude_code_version`, `request_id`, `cache_ttl_split` | Extend `SessionAggregate` to carry the new fields. Detect model-flips by scanning the assistant timeline for non-monotonic model changes (`DATA-CLAUDE.md` §2.9). Populate `cache_ttl_split` by reading `usage.cache_creation.ephemeral_5m_input_tokens` and `ephemeral_1h_input_tokens` from `usage` (currently only the rollup is read at `174`). **TOKENS.md §8 #3.** | `packages/tools/db90-claude/src/claude-reader.ts:38-53, 155-225, 244-256`, `packages/tools/db90-claude/src/pricing.ts:33-171` (to consume `cache_ttl_split` at the 2× rate for 1h tokens) |
...
| `claude-10` | Front-end | Typed `ToolEventMetadata.claude` discriminated union | Per §11: extend the `ToolEvent`/`ToolEventMetadata` types so consumers can read `metadata.advisor_model` / `metadata.subagent_type` / `metadata.tool_name_inner` without `unknown` casting. Discriminate by `tool_name === "claude_code"`. | `packages/web/src/lib/types.ts:224-250` |
...
| `claude-12` | Front-end | Advisor cost decomposition tile | When `metadata.advisor_model` is present on session-level events, render a cost split (Sonnet base + Opus advisor) on the events page detail drawer and on the org dashboard. Uses `formatCost` per the CLAUDE.md formatter rule. | `packages/web/src/components/events/EventDrawer.tsx`, `packages/web/src/components/events/EventDetail.tsx`, `packages/web/src/pages/OrgDashboard.tsx` |

**Group coverage:** Schema (1) · Extractor (5) · Sanitization (3) · Front-end (3) — all four groups, twelve total. Exceeds the ≥ 6 / ≥ 3-of-4 bar.
...
| `last-seen model wins` semantic + advisor turns | A Sonnet executor session with two Opus advisor turns ends with `agg.model = "claude-opus-4-7"` if the Opus turn is last. All session tokens are then billed at Opus rates — inflating the session's recorded cost by ~5× vs the real Sonnet+Opus mix. | `claude-3` adds `metadata.advisor_model` (detected from model-flips) without changing `agg.model`. A follow-up to `claude-5` should switch `calculateCost` to a weighted-by-turn computation when `cache_ttl_split` is augmented with per-model token counts. Stage this gracefully — out of scope for §13's twelve items. |
...
| Model attribution | `unknown` on aggregates (Paths A/B), real on Path C (legacy rows) (§1a, §1c) | Real on every event but **last-seen wins** within a session (`claude-reader.ts:199-200`), so advisor turns flip `agg.model` to Opus and rebill the session (§8b, §14 row 4) | Cursor: no near-term fix without composer parsing (`cursor-5`). Claude: `claude-3` (`metadata.advisor_model`) captures the model-flip; downstream a weighted-by-turn cost is a follow-up to `claude-5`. |
