# Manual end-to-end checklist — @aixle/insights + Claude Code
...
1. **Install / run the package**
   - From any directory: `npx -y @aixle/insights init --keycloak-url <ISSUER> [--host <API>]`
...
3. **Confirm CLI outcome**
   - Expect: credentials saved, Claude user MCP config updated under `~/.claude.json` (top-level `mcpServers.aixle-insights`), and the line: `Restart Claude Code to activate`.

4. **Restart Claude Code**
   - Fully quit and reopen Claude Code.

5. **Verify MCP**
   - In Claude Code, open `/mcp` (or the MCP tools panel) and confirm **aixle-insights** is listed.
...
- **`init` reports a conflicting `aixle-insights` MCP entry:** Inspect `~/.claude.json` → `mcpServers.aixle-insights`. Re-run with `init --force` only if you intend to replace that entry. (A legacy `mcpServers.db90` entry from an older install is migrated/removed automatically.)
- **MCP not listed after restart:** Confirm `~/.claude.json` exists and contains `mcpServers.aixle-insights` with `command`/`args` as installed by `init` (on Windows, `command` is `cmd` and `args` begin with `/c`, `npx`, …).
