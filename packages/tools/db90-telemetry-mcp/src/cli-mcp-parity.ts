 ...
 */
import type { CollectedCursorPayloads } from "./collect-cursor-payloads.js";
...
export interface CliCursorCollectedPayloads {
...
export interface McpCursorCollectedPayloads extends CollectedCursorPayloads {
  mcp_transcript_mode: boolean;
...
export interface CliMcpParityReport {
  ...
  cli: CliCursorCollectedPayloads;
  mcp: McpCursorCollectedPayloads;
...
export function inferParityPath(payload: CursorDb90Payload): ParityPath {
  if (payload.metadata.transcript_source === "agent_transcript") return "mcp_transcript";
  return inferIngestPath(payload);
}

export function countByParityPath(payloads: CursorDb90Payload[]): ParityPathCounts {
