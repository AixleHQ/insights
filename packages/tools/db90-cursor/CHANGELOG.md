...
- Library entry `@db90/cursor/sync` exposes `syncOnce(options: SyncOptions): Promise<SyncResult>` for direct use by `@db90/telemetry-mcp`. Re-exports `resolveProjectId`, `ProjectResolution`, `PricingConfig`, `DEFAULT_PRICING`.
