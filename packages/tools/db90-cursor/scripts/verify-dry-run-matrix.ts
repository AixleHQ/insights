 ...
 *
 * Usage: npm run verify:dry-run-matrix
 */
...
function redactPayload(payload: Db90Payload): Db90Payload {
  const home = homedir();
...
function main(): void {
  ...
  if (!probeCursorGlobalStateDb(true)) {
    console.error(
      "\nSQLite probe failed. From repo root try:\n  cd packages/tools && npm rebuild better-sqlite3\nThen re-run: npm run verify:dry-run-matrix"
    );
    process.exit(1);
  }

  const { payloads, counts } = collectSyncPayloads({ since: null, verbose: true });
  ...
  const samplesByPath: Record<string, Db90Payload[]> = {};
  ...
  console.log("\nAll payloads match DATA-CURSOR.md §3.5 contract.");
}
