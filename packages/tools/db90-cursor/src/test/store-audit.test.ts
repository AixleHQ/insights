...
describe("auditCursorLocalStores", () => {
  it("returns no_legacy_dbs when only state.vscdb exists", () => {
    const root = mkdtempSync(join(tmpdir(), "db90-full-audit-"));
    const globalDb = join(root, "globalStorage", "state.vscdb");
    createItemTableDb(globalDb, [
      {
        key: "aiCodeTracking.dailyStats.v1.5.2026-05-20",
        value: JSON.stringify({ composerSuggestedLines: 5, composerAcceptedLines: 3 }),
      },
    ]);

    const report = auditCursorLocalStores(root);
    expect(report.path_c_verdict).toBe("no_legacy_dbs");
    expect(report.legacy_cursor_db.count).toBe(0);
    expect(report.state_vscdb.global.daily_stats_key_count).toBe(1);
    expect(report.daily_stats_versions.buckets).toEqual([
      expect.objectContaining({ version: "v1.5", key_count: 1 }),
    ]);
    expect(report.daily_stats_versions.has_version_newer_than_v1_5).toBe(false);
  });

  it("returns legacy_has_rows when cursor.db has feedback rows", () => {
    const root = mkdtempSync(join(tmpdir(), "db90-legacy-audit-"));
  ...
  it("returns legacy_present_empty when cursor.db exists without rows", () => {
    const root = mkdtempSync(join(tmpdir(), "db90-empty-legacy-audit-"));
    createItemTableDb(join(root, "globalStorage", "state.vscdb"), []);
    const dbPath = join(root, "workspaceStorage", "ws1", "cursor.db");
    mkdirSync(join(dbPath, ".."), { recursive: true });
    writeFileSync(dbPath, "");
