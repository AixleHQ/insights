...
function payload(partial: Partial<Db90Payload> & { metadata?: Record<string, unknown> }): Db90Payload {
  return {
    tool_name: "cursor",
    event_type: "chat",
    model: "unknown",
...
function emptyCollected(): CollectedPayloads {
  return {
    payloads: [],
    counts: {
      legacy: 0,
...
describe("buildCliMcpParityReport", () => {
  it("flags cli_only paths as MCP-only gaps", () => {
    const cli: CollectedPayloads = {
      ...emptyCollected(),
      payloads: [
        payload({ event_type: "completion", metadata: { cursor_session_id: null, workspace: "/w", cost_model: "estimated_line_count", scannable: false, risk_level: "none" } }),
      ],
      counts: { ...emptyCollected().counts, legacy: 0, dailyStatsEntries: 1 },
    };
    const mcp = { ...emptyCollected(), mcp_transcript_mode: false, transcript_files: 0 };
