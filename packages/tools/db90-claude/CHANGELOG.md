...
- Library entry `@db90/claude/sync` exposes `syncOnce(options: SyncOptions): Promise<SyncResult>` for direct use by `@db90/telemetry-mcp`. Re-exports `PricingTable`, `ModelPricing`, `DEFAULT_PRICING`, `mergePricing`, `resolveProjectId`, `ProjectResolution`.
...
- Breaking changes to `syncOnce`, `SyncOptions`, or `SyncResult` require a minor-version bump on this package and a coordinated dep bump in `@db90/telemetry-mcp`.
