...
class UpdateToolEventsRetentionPolicy < ActiveRecord::Migration[8.1]
  ...
  def up
    return unless tool_events_hypertable?
  ...
  # Skip when TimescaleDB is absent, or tool_events exists but is not a hypertable
  # (e.g. plain table / different schema in dev or forked DBs).
  def tool_events_hypertable?
    sql = <<~SQL.squish
      SELECT EXISTS (
        SELECT 1
        FROM timescaledb_information.hypertables
        WHERE hypertable_schema = 'timeseries'
          AND hypertable_name = 'tool_events'
      )
    SQL
