...
# AIX-350: Fix inflated tokens_in for historical rows with cache token metadata.
#
# Previously, tokens_in included cache_write + cache_read tokens (the Anthropic API
...
# - Stores original tokens_in in metadata['_pre_350_tokens_in'] for rollback.
class BackfillCacheTokenAccuracy < ActiveRecord::Migration[8.0]
    ...
    loop do
      rows_affected = exec_update(<<~SQL)
        ...
        SET
          ...
          metadata = te.metadata || jsonb_build_object('_pre_350_tokens_in', te.tokens_in)
        FROM batch
    ...
    loop do
      rows_affected = exec_update(<<~SQL)
        WITH batch AS (
          SELECT id, occurred_at
          FROM timeseries.tool_events
          WHERE (metadata->>'_pre_350_tokens_in') IS NOT NULL
          LIMIT #{BATCH_SIZE}
        )
        UPDATE timeseries.tool_events te
        SET
          tokens_in = (te.metadata->>'_pre_350_tokens_in')::integer,
          tokens_total = (te.metadata->>'_pre_350_tokens_in')::integer + COALESCE(te.tokens_out, 0),
          metadata = te.metadata - '_pre_350_tokens_in'
        FROM batch
        WHERE te.id = batch.id AND te.occurred_at = batch.occurred_at
      SQL
