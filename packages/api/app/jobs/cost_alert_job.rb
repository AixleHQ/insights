...
class CostAlertJob
  ...
  def check_organization_thresholds(org)
    ...
    if thresholds[:daily] && daily_cost(org) >= thresholds[:daily]
      send_alert(org, :daily, daily_cost(org), thresholds[:daily])
      alerts_sent += 1
    end

    # Check weekly threshold
    if thresholds[:weekly] && weekly_cost(org) >= thresholds[:weekly]
      send_alert(org, :weekly, weekly_cost(org), thresholds[:weekly])
      alerts_sent += 1
    end

    # Check monthly threshold
    if thresholds[:monthly] && monthly_cost(org) >= thresholds[:monthly]
      send_alert(org, :monthly, monthly_cost(org), thresholds[:monthly])
      alerts_sent += 1
    end

    # Check per-user thresholds
    alerts_sent += check_user_thresholds(org, thresholds)

    alerts_sent
  end

  def load_thresholds(org)
    settings = org.organization_settings.where(key: %w[cost_threshold_daily cost_threshold_weekly cost_threshold_monthly cost_threshold_per_user])
                  .pluck(:key, :value)
                  .to_h
  ...
  def check_user_thresholds(org, thresholds)
    ...
    user_costs.each do |user_id, cost|
      if cost && cost >= thresholds[:per_user]
        send_user_alert(org, user_id, cost, thresholds[:per_user])
        alerts_sent += 1
      end
    end
  ...
  def send_alert(org, period, current_cost, threshold)
  ...
  def send_user_alert(org, user_id, current_cost, threshold)
