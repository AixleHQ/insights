...
class ProjectAuditLogPolicy < ApplicationPolicy
  ...
  relation_scope do |scope|
    if global_admin?
      ...
      org_admin_project_ids = Project.where(
        organization_id: user.organization_memberships.admins.select(:organization_id)
      ).select(:id)
      personal_project_ids = Project.where(owner: user, organization: nil).select(:id)
      scope.where(project_id: project_admin_ids)
           .or(scope.where(project_id: org_admin_project_ids))
           .or(scope.where(project_id: personal_project_ids))
    else
      scope.none
    end
  end
